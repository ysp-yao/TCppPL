...
struct Matrix_slice {
  ...
  {
    size = 1;
    for (const size_t item : extents)
      size *= item;
    strides[0] = sizeof(T);
    strides[1] = sizeof(T)*extents[1];
   }
  ...
  size_t size=0; // total number of elements
  size_t start; // starting offset
  array<size_t, N> extents; // number of elements in each dimension, 比如[2, 3]表示一个2x3的矩阵
  array<size_t, N> strides; // offsets between elements in each dimension,，比如2x3的矩阵，元素类型为T，则值为[sizeof(T), 3*sizeof(T)]
};
...
struct Matrix_slice<2> {
  ...
  size_t size = 0; // total number of elements
  size_t start; // starting offset
  array<size_t, 2> extents; // number of elements in each dimension, 比如[2, 3]表示一个2x3的矩阵
  array<size_t, 2> strides; // offsets between elements in each dimension,，比如2x3的矩阵，元素类型为T，则值为[sizeof(T), 3*sizeof(T)]
...
struct Matrix_slice<1> {
  ...
  size_t start; // starting offset
  array<size_t, 2> extents; // number of elements in each dimension, 比如[2, 3]表示一个2x3的矩阵
  array<size_t, 1> strides; // offsets between elements in each dimension,，比如2x3的矩阵，元素类型为T，则值为[sizeof(T), 3*sizeof(T)]
...
public:
  ...
  Matrix_ref(Matrix_slice<N> ms, T* p) {
    desc = ms;
    ptr = p;
  }

  Matrix_slice<N> GetDesc() const { return desc; }

  T* begin() const { return ptr; }
  T* end() const { return ptr + desc.size; }
...
public:
  ...
  Matrix_ref<T, N - 1> row(size_t n) {
    ...
    Matrix_impl::slice_dim_0(n,desc,row);
    return {row,data()};
  }
  ...
  size_t rows() { return desc.extents[0]; } // 行数
...
Matrix<T, N>::Matrix(Matrix_initializer<T, N> init) {
  ...
  desc.size = 1;
  for (const size_t item : desc.extents)
    desc.size *= item;
  desc.strides[0] = sizeof(T);
  desc.strides[1] = sizeof(T)*desc.extents[1];
...
Matrix<T, N>::Matrix(const Matrix_ref<U, N>& x)
  ...
  elems{ x.begin(), x.end() }
{
  std::cout << "Matrix(const Matrix_ref<U, N>& x)" << "\n";
...
template<typename T, size_t N>
  ...
  {
    std::cout << "Matrix<T, N>::operator=(const Matrix_ref<U, N>& x)\n";
